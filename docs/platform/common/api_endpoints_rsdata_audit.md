...
| POST | `/internal/users/validate` | `ResponseEntity<RsData<Void>>` | 200 | O |
...
| POST | `/api/v1/test/cache-invalidation` | `ResponseEntity<RsData<String>>` | 200 | O |
...
| GET | `/internal/events/{eventId}/version-check` | `ResponseEntity<Boolean>` | 200 | X |
...
- `user` 모듈은 `ValidationErrors`에 대해서만 RsData로 처리하며, 그 외 예외는 기본 오류 응답(비 RsData)일 수 있음.
- `seat`, `purchase`, `notification`, `app` 모듈에는 RsData 기반 전역 예외 처리기가 없어, 런타임 예외 시 기본 오류 응답(비 RsData)으로 내려갈 가능성이 있음.
