...
현재 레포에는 `batch` 모듈과 `/api/batch/**` 엔드포인트 구현이 없으므로, 배치 부분은 **외부 서비스 또는 향후 도입을 전제로 한 예시**입니다.
...
### 2. 배치 시스템
- **batch 모듈**: Spring Batch 기반 ANALYZE 작업 실행 (예시)
...
```bash
# 현재 레포에는 :batch 모듈이 없으므로 외부 배치 서비스에서 실행합니다.
```
