...
현재는 라우팅/화이트리스트를 별도 파일로 분리하고 `application.yml`에서 import 합니다.
...
spring:
  ...
  config:
    import:
      ...
      - optional:classpath:/application-gateway-routes-local.yml
      - optional:classpath:/application-gateway-routes-prod.yml
      - optional:classpath:/application-whitelist-local.yml
      - optional:classpath:/application-whitelist-prod.yml
```

#### 로컬 라우팅 (`application-gateway-routes-local.yml`)
...
#### 화이트리스트 (`application-whitelist-local.yml`)
...
- 동적 라우팅: 유레카에 등록된 서비스들의 실시간 상태를 확인하고, 가장 적절한 마이크로서비스 인스턴스로 요청을 전달합니다. (예: lb://broker)
