...
- Controller: `NotificationCommandController.createNotification()`
- Actor: 관리자/매니저
...
- 요청은 `POST` `/api/v1/notifications` 기준으로 해석한다.
- 컨트롤러는 자신이 맡은 입력 검증과 서비스 호출까지만 책임진다.
- 응답은 `ResponseEntity<RsData<NotificationDto>>` 기준으로 외부 계약을 유지한다.

## Related Docs

- [Flow](../../flow/notification/NotificationCommandController/createNotification.md)
