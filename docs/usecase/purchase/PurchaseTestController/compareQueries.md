...
- Controller: `PurchaseTestController.compareQueries()`
- Actor: 테스트
...
- 요청은 `GET` `/api/test/purchase/compare/{eventId}` 기준으로 해석한다.
- 컨트롤러는 자신이 맡은 입력 검증과 서비스 호출까지만 책임진다.
- 응답은 `ResponseEntity<RsData<String>>` 기준으로 외부 계약을 유지한다.

## Related Docs

- [Flow](../../flow/purchase/PurchaseTestController/compareQueries.md)
