# POST /api/v1/events/image/url Flow

## Entry Point

- `ImageUploadController.generateImageUploadUrls()`
- `POST /api/v1/events/image/url`

## Flow

- `ImageUploadController.generateImageUploadUrls()`가 `POST /api/v1/events/image/url`를 처리한다.
