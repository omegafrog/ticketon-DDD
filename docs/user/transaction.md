...
`auth`에서 `SecurityUser` 생성이 실패하여 `user-registered-failed` 이벤트가 발행되면, 이를 수신하는 `user`의 Consumer가 `User`의 상태를 `FAILED`로 업데이트합니다. (현재는 `cancelUserRegistration`을 호출하여 삭제하고 계십니다. 상태 관리 방식에서는 삭제 대신 상태를 변경합니다.)
...
`auth`에서 `SecurityUser` 생성이 성공했을 때도 이 사실을 `user`에 알려주어 `User`의 상태를 `COMPLETED`로 변경해야 합니다.

*   `auth`의 `UserRegisteredEventConsumer`에서 `SecurityUser` 생성 성공 후, `security-user-registered-successfully` 같은 성공 이벤트를 발행합니다.
*   `user`에서 이 성공 이벤트를 수신하는 새로운 Consumer를 만들어 `User`의 상태를 `COMPLETED`로 업데이트합니다.
...
*   **클라이언트**: 현재 코드베이스에는 가입 상태를 조회하는 전용 API가 없습니다.  
    가입 상태 폴링이 필요하다면 `GET /api/users/{userId}/status` 같은 엔드포인트를 별도 설계/구현해야 합니다.

### 요약

| 단계 | 서비스 | 작업 내용 | 상태 변경 |
| :--- | :--- | :--- | :--- |
| 1 | `user` | 회원가입 API 요청 받음 | User: **PENDING** |
| 2 | `user` | `user-registered` 이벤트 발행 | |
| 3 | `auth` | 이벤트 수신, `SecurityUser` 생성 | |
| 4a (실패) | `auth` | `user-registered-failed` 이벤트 발행 | |
| 5a (실패) | `user` | 이벤트 수신, User 삭제 또는 상태 변경 | User: **FAILED** |
| 4b (성공) | `auth` | `security-user-registered-successfully` 이벤트 발행 | |
| 5b (성공) | `user` | 이벤트 수신, User 상태 변경 | User: **COMPLETED** |
