# EntryPromoter의 주기성 보장

EntryPromoter는 일정 시간 간격마다 waiting queue의 데이터를 읽어 count만큼 entry queue로 옮기는 작업을 진행한다.
...
## 개선 방법
### 1. 스케쥴러의 성능 개선
기존 로직은 `SCAN WAITING:*` 방식으로 모든 이벤트의 대기열 키를 순회한다. 키가 많아질수록 비용이 커질 수 있다.
