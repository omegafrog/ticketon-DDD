...
#### 비정상 종료 시 메시지 재처리 로직 (현재 미구현)
Redis 재시작 시 pending 메시지를 재처리하는 로직은 **현재 코드에 구현되어 있지 않습니다**.

- Broker의 `EntryStreamMessageListener`는 스트림 리스너 시작만 수행합니다.
- Dispatcher의 `EntryQueueConsumer`에는 pending 재처리 코드가 있으나 **주석 처리되어 비활성화**되어 있습니다.

따라서 재처리 동작은 문서상의 설계 방향이며, 실제 동작은 구현 상태에 맞춰 추후 반영이 필요합니다.
