...
public class NewEventRequest {
    ...
    public NewEventRequest(String title, EventCategoryId categoryId, String description,
        ...
        this.minPrice =
            seatLayout.getSeats().stream().map(seat -> seat.getPrice()).min((a, b) -> a - b)
                .orElse(0);
        this.maxPrice =
            seatLayout.getSeats().stream().map(seat -> seat.getPrice()).max((a, b) -> a - b)
                .orElse(0);
        validate();
    }
    ...
    private void validate() {
        ...
        if (startDate.isAfter(endDate)) {
            fieldErrors
                .add(new FieldError("startDate", startDate.toString(),
