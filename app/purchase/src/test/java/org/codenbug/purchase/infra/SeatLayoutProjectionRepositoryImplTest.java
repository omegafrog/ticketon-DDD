...
public class SeatLayoutProjectionRepositoryImplTest {
    ...
    void testSaveAndFindSeatLayoutProjection() {
        ...
        Set<Seat> seats = new java.util.HashSet<>(Set.of(new Seat("seat-1", "A-01", 50000, "VIP"),
                new Seat("seat-2", "A-02", 50000, "VIP"),
                new Seat("seat-3", "B-01", 30000, "STANDARD")));

        SeatLayoutProjection projection =
                new SeatLayoutProjection(layoutId, layout, locationName, hallName, seats);
        ...
        assertTrue(foundSeats.stream().anyMatch(
                seat -> "seat-1".equals(seat.getSeatId()) && "A-01".equals(seat.getSignature())));
        assertTrue(foundSeats.stream()
                .anyMatch(seat -> "seat-3".equals(seat.getSeatId()) && seat.getAmount() == 30000));
    }
    ...
    void testExistsByIdMethod() {
        ...
        Long layoutId = 200L;
        SeatLayoutProjection projection = new SeatLayoutProjection(layoutId, "Exist Test Layout",
                "Test Location", "Test Hall", Set.of());
    ...
    void testRepositoryImplWrapper() {
        ...
        SeatLayoutProjectionRepositoryImpl repositoryImpl =
                new SeatLayoutProjectionRepositoryImpl(jpaRepository);

        Long layoutId = 300L;
        Set<Seat> seats = new java.util.HashSet<>(
                Set.of(new Seat("wrapper-seat-1", "W-01", 25000, "ECONOMY")));

        SeatLayoutProjection projection = new SeatLayoutProjection(layoutId, "Wrapper Test Layout",
                "Wrapper Location", "Wrapper Hall", seats);
    ...
    void testUpdateSeatLayoutProjection() {
        ...
        Long layoutId = 400L;
        Set<Seat> originalSeats =
                new java.util.HashSet<>(Set.of(new Seat("original-seat", "O-01", 10000, "BASIC")));

        SeatLayoutProjection projection = new SeatLayoutProjection(layoutId, "Original Layout",
                "Original Location", "Original Hall", originalSeats);
        ...
        Set<Seat> updatedSeats =
                new java.util.HashSet<>(Set.of(new Seat("updated-seat-1", "U-01", 20000, "UPDATED"),
                        new Seat("updated-seat-2", "U-02", 20000, "UPDATED")));

        foundProjection.updateFrom("Updated Layout", "Updated Location", "Updated Hall",
                updatedSeats);
