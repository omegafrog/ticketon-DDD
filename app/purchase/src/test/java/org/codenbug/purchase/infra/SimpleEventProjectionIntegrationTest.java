...
/**
 * 간단한 Kafka 통합 테스트 EventProjectionConsumer의 핵심 동작을 검증합니다.
 */
@DataJpaTest
...
public class SimpleEventProjectionIntegrationTest {
    ...
    void testEventCreatedKafkaIntegration() throws InterruptedException {
        ...
        Map<String, Object> consumerProps =
                KafkaTestUtils.consumerProps("simple-test-group", "true", embeddedKafkaBroker);
        ...
        DefaultKafkaConsumerFactory<String, EventCreatedEvent> consumerFactory =
                new DefaultKafkaConsumerFactory<>(consumerProps);

        // Given: EventProjectionConsumer 생성
        EventProjectionConsumer eventProjectionConsumer =
                new EventProjectionConsumer(jpaRepository);
        CountDownLatch latch = new CountDownLatch(1);

        // Given: 메시지 리스너 컨테이너 설정
        ContainerProperties containerProperties = new ContainerProperties("event-created");
        containerProperties
                .setMessageListener((MessageListener<String, EventCreatedEvent>) record -> {
                    try {
    ...
    void testDirectConsumerIntegration() {
        ...
        EventCreatedEvent event = new EventCreatedEvent(eventId, "Direct Test Event",
                "direct-manager", 100L, true, "Direct Test Location", "2024-12-25T19:00:00",
                "2024-12-25T22:00:00", 0, 10000, 1L);
