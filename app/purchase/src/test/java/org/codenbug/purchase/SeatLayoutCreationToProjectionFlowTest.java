 ...
 * 
 * 플로우: 1. Seat 모듈에서 SeatLayoutCreatedEvent 발행 (시뮬레이션) 2. Purchase 모듈의 SeatLayoutProjectionConsumer가
 * 이벤트 소비 3. SeatLayoutProjection 엔티티 생성 및 DB 저장 4. Repository를 통한 조회 및 검증
 */
@DataJpaTest
...
public class SeatLayoutCreationToProjectionFlowTest {
    ...
    void testCompleteSeatLayoutCreationToProjectionFlow() {
        ...
        SeatLayoutProjectionRepository seatLayoutProjectionRepository =
                new SeatLayoutProjectionRepositoryImpl(jpaRepository);
        SeatLayoutProjectionConsumer seatLayoutProjectionConsumer =
                new SeatLayoutProjectionConsumer(jpaRepository);
        ...
        List<SeatLayoutCreatedEvent.SeatInfo> seats = List.of(
                ...
                new SeatLayoutCreatedEvent.SeatInfo("seat-premium-01", "P-B01", 100000, "PREMIUM",
                        true),
                new SeatLayoutCreatedEvent.SeatInfo("seat-premium-02", "P-B02", 100000, "PREMIUM",
                        true),
                new SeatLayoutCreatedEvent.SeatInfo("seat-standard-01", "S-C01", 50000, "STANDARD",
                        true),
                new SeatLayoutCreatedEvent.SeatInfo("seat-standard-02", "S-C02", 50000, "STANDARD",
                        true));

        // 1단계: Seat 모듈에서 SeatLayoutCreatedEvent 생성 (실제로는 ApplicationEventPublisher를 통해 발행)
        SeatLayoutCreatedEvent seatLayoutCreatedEvent =
        ...
        assertAll("SeatLayout 생성부터 SeatLayoutProjection 저장까지 전체 플로우 검증",
                ...
                () -> assertEquals(layout, savedProjection.getLayout(), "레이아웃 정보가 정확히 저장되어야 합니다"),
                () -> assertEquals(locationName, savedProjection.getLocationName(),
                        "위치명이 정확히 저장되어야 합니다"),
                () -> assertEquals(hallName, savedProjection.getHallName(), "홀명이 정확히 저장되어야 합니다"),
                () -> assertEquals(6, savedProjection.getSeats().size(), "좌석 수가 정확히 저장되어야 합니다"));
        ...
        assertTrue(
                savedSeats.stream()
                        .anyMatch(seat -> "seat-vip-01".equals(seat.getSeatId())
                                && "VIP-A01".equals(seat.getSignature())
                                && seat.getAmount() == 150000 && "VIP".equals(seat.getGrade())),
                "VIP 좌석 정보가 정확히 저장되어야 합니다");

        // PREMIUM 좌석 검증
        assertTrue(
                savedSeats.stream()
                        .anyMatch(seat -> "seat-premium-01".equals(seat.getSeatId())
                                && "P-B01".equals(seat.getSignature()) && seat.getAmount() == 100000
        ...
        assertTrue(
                savedSeats.stream()
                        .anyMatch(seat -> "seat-standard-01".equals(seat.getSeatId())
                                && "S-C01".equals(seat.getSignature()) && seat.getAmount() == 50000
                                && "STANDARD".equals(seat.getGrade())),
                "STANDARD 좌석 정보가 정확히 저장되어야 합니다");

        // 추가 검증: JPA를 통한 직접 조회로도 확인
        SeatLayoutProjection directlyFoundProjection =
                jpaRepository.findById(layoutId).orElse(null);
    ...
    void testMultipleSeatLayoutCreationScenario() {
        ...
        SeatLayoutCreatedEvent layout1 =
                new SeatLayoutCreatedEvent(100L, "카페 공연장 배치", "홍대 카페거리", "소공연장",
                        List.of(new SeatLayoutCreatedEvent.SeatInfo("cafe-01", "T-01", 25000,
                                "TABLE", true),
                                new SeatLayoutCreatedEvent.SeatInfo("cafe-02", "T-02", 25000,
                                        "TABLE", true)));

        // 두 번째 레이아웃: 대형 야외 무대
        SeatLayoutCreatedEvent layout2 =
                new SeatLayoutCreatedEvent(200L, "야외 페스티벌 무대", "한강공원", "야외무대",
                        List.of(new SeatLayoutCreatedEvent.SeatInfo("outdoor-vip-01", "V-01", 80000,
                                "VIP", true),
                                new SeatLayoutCreatedEvent.SeatInfo("outdoor-general-01", "G-01",
                                        40000, "GENERAL", true),
                                new SeatLayoutCreatedEvent.SeatInfo("outdoor-general-02", "G-02",
                                        40000, "GENERAL", true),
                                new SeatLayoutCreatedEvent.SeatInfo("outdoor-general-03", "G-03",
                                        40000, "GENERAL", true)));

        // 세 번째 레이아웃: 클래식 콘서트홀
        SeatLayoutCreatedEvent layout3 =
                new SeatLayoutCreatedEvent(300L, "클래식 콘서트홀 배치", "예술의전당", "콘서트홀",
                        List.of(new SeatLayoutCreatedEvent.SeatInfo("classic-royal-01", "R-01",
                                200000, "ROYAL", true),
                                new SeatLayoutCreatedEvent.SeatInfo("classic-vip-01", "V-01",
                                        120000, "VIP", true),
                                new SeatLayoutCreatedEvent.SeatInfo("classic-premium-01", "P-01",
                                        80000, "PREMIUM", true)));
        ...
        // 특정 좌석 정보 검증
        assertTrue(classicLayout.getSeats().stream().anyMatch(
                seat -> "classic-royal-01".equals(seat.getSeatId()) && seat.getAmount() == 200000),
