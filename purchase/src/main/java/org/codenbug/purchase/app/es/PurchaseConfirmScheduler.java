...
public class PurchaseConfirmScheduler {
	...
	public void processPendingConfirms() {
		...
		for (PurchaseOutboxMessage msg : batch) {
			if (!PurchaseConfirmCommandService.CONFIRM_WORK_QUEUE.equals(msg.getQueueName())) {
				continue;
			}
			...
			if (isTerminalStatus(currentStatus)) {
				msg.markPublishAttemptFailed("non-retryable status: " + currentStatus.name());
			...
			if (msg.getPublishAttempts() >= maxPublishAttempts) {
				...
				msg.markPublishAttemptFailed("max publish attempts exceeded");
				msg.markPublished(LocalDateTime.now());
				outboxRepository.save(msg);
				continue;
			}
