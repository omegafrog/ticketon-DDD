...
export BASE_URL OUTPUT_FILE MYSQL_HOST MYSQL_PORT MYSQL_USER MYSQL_PASSWORD MYSQL_DATABASE
...
def insert_purchase_chunk(chunk):
    values = []
    for row in chunk:
        values.append("(" + ", ".join([
...
for start in range(0, len(rows), chunk_size):
    ...
    insert_purchase_chunk(chunk)
    print(f"purchase inserted {start + len(chunk)}/{len(rows)}")
