...
const SSE_TIMEOUT = `${parseInt(__ENV.SSE_TIMEOUT_MS || '120000', 10)}ms`;
...
export const options = {
    thresholds: {
        ...
        sse_connection_time: [`p(95)<${__ENV.SSE_P95_THRESHOLD || 5000}`],
...
const sseFailures = new Counter('sse_failures');
const disconnectFailures = new Counter('disconnect_failures');
const completedUsers = new Counter('completed_users');
const inEntryCount = new Counter('in_entry_events');
const inProgressBeforeEntryCount = new Counter('in_progress_before_entry_events');
const sseEventCount = new Counter('sse_events');
...
async function getWaitingKeyCount(waitingKey) {
    // WAITING may be implemented as ZSET or HASH depending on branch.
    // This helper makes the harness branch-agnostic and avoids WRONGTYPE.
    let type;
    try {
        type = String(await redisClient.sendCommand('TYPE', waitingKey) || '').trim();
    } catch (e) {
        fail(`Redis TYPE failed for key=${waitingKey}: ${e}`);
    }
    ...
    if (type === 'zset') {
        return Number(await redisClient.sendCommand('ZCARD', waitingKey)) || 0;
    }
    if (type === 'hash') {
        return Number(await redisClient.sendCommand('HLEN', waitingKey)) || 0;
    }
    fail(`Unsupported WAITING key type for key=${waitingKey}: type=${type}`);
}

async function getWaitingQueueIdxTotal() {
    // WAITING_QUEUE_IDX is a HASH: eventId -> idx (monotonic counter).
    // We use this as the admin barrier signal because it does not shrink like WAITING itself.
    let total = 0;
    for (const eventId of EVENT_IDS) {
        try {
            const v = await redisClient.sendCommand('HGET', 'WAITING_QUEUE_IDX', eventId);
            total += Number(v) || 0;
        } catch (_) {
...
async function safeHgetNumber(hashKey, field) {
    try {
        const v = await redisClient.sendCommand('HGET', hashKey, field);
        return Number(v) || 0;
    } catch (e) {
        // xk6-redis may surface missing values as an error (e.g., "redis: nil").
        // For benchmark preconditions, treat missing as 0.
        return 0;
    }
}
...
async function getWaitingTotal() {
    ...
    for (const eventId of EVENT_IDS) {
        try {
            total += await getWaitingKeyCount(buildWaitingKey(eventId));
        } catch (_) {
            // treat missing keys/errors as 0
        }
    }
...
async function getWaitingCountsByEvent() {
    ...
    for (const eventId of EVENT_IDS) {
        ...
        counts[eventId] = await getWaitingKeyCount(key);
    }
    return counts;
}
...
function connectSse(eventId, accessToken, jar, runId) {
    if (VERBOSE_LOGS) {
        console.log(`[connectSse] Starting connection for eventId: ${eventId}`);
    }
    const params = {
        headers: {
            Authorization: `Bearer ${accessToken}`,
    ...
    let inEntryReceived = false;
    let promoted = false;
    let sseClosed = false;
    let done = false;

    const startConnect = Date.now();
    const res = sse.open(`${BROKER_BASE_URL}/api/v1/broker/events/${eventId}/tickets/waiting`, params, (client) => {
        client.on('open', () => {
            if (VERBOSE_LOGS) {
                console.log(`[connectSse] SSE connection opened for eventId: ${eventId}`);
            }
        });
        ...
        client.on('event', async (event) => {
            ...
            if (VERBOSE_LOGS) {
                console.log(`[connectSse] Received SSE event for eventId: ${eventId}, data: ${event.data}`);
            }
            if (!event || !event.data) {
                return;
            }
            try {
                const rawData = normalizeSseData(event.data);
                let parsed;
                try {
                    parsed = JSON.parse(rawData);
                    if (VERBOSE_LOGS) {
                        console.log(`[connectSse] Parsed SSE event for eventId: ${eventId}, status: ${parsed.status}`);
                    }
                    sseEventCount.add(1);
                } catch (e) {
                    if (VERBOSE_LOGS) {
                        console.log(`[connectSse] Failed to parse SSE event data as JSON for eventId: ${eventId}, error: ${e}`);
                    }
                    return;
                }

                if (parsed.status === 'IN_ENTRY') {
                    if (!inEntryReceived) { // Only process once per VU
                        if (VERBOSE_LOGS) {
                            console.log(`[connectSse] IN_ENTRY received for eventId: ${eventId}. Waiting for IN_PROGRESS...`);
                        }
                        inEntryReceived = true;
                        inEntryCount.add(1);
                    }
                } else if (parsed.status === 'IN_PROGRESS') {
                    done = true;
                    if (!inEntryReceived) {
                        inProgressBeforeEntryCount.add(1);
                        inEntryReceived = true;
                        if (VERBOSE_LOGS) {
                            console.log(`[connectSse] IN_PROGRESS received for eventId: ${eventId} BEFORE IN_ENTRY. Treating as promoted.`);
                        }
                    }

                    if (VERBOSE_LOGS) {
                        console.log(`[connectSse] IN_PROGRESS received for eventId: ${eventId}. Disconnecting.`);
                    }
                    ...
                    //     try {
                    //         console.log("hi")
                    //         console.log(buildOpenAtKey(runId))
                    //         const openAtRaw = await redisClient.get(buildOpenAtKey(runId));
                    //         console.log("bye", openAtRaw)
                    //         const openAt = Number(openAtRaw) || 0;
                    //         if (openAt > 0) {
                    //             openToPromoteMs.add(Date.now() - openAt);
                    //         }
                    //     } catch (e) {
                    //         console.log(e)
                    //         // ignore
                    ...
                    // Close SSE first to stop receiving events, then best-effort disconnect (bounded timeout).
                    console.log(`[connectSse] Disconnecting start for eventId: ${eventId}.`);
                    client.close();
                    console.log("disconnected")
                    disconnect(eventId, accessToken, jar);
                    promoted = true;
                    sseClosed = true;
                }
            } catch (e) {
                if (VERBOSE_LOGS) {
                    console.log(`[connectSse] Error in SSE event handler for eventId: ${eventId}, error: ${e}`);
                }
                // ignore
            }
        });
        ...
        client.on('error', (e) => {
            if (VERBOSE_LOGS) {
                console.log(`[connectSse] SSE connection error for eventId: ${eventId}, error: ${e.error()}`);
            }
            if (!promoted) {
                sseFailures.add(1);
            }
            sseClosed = true;
            // The sse.open call will naturally terminate upon error or server-side close.
        });
    });
    if (VERBOSE_LOGS) {
        console.log(res);
    ...
    if (!checkOk) {
        sseFailures.add(1);
        if (VERBOSE_LOGS) {
            console.log(`[connectSse] Initial SSE connection check failed for eventId: ${eventId}`);
        }
        return false;
    }
    ...
    return inEntryReceived;
}

function normalizeSseData(raw) {
...
function disconnect(eventId, accessToken, jar) {
    const res = http.post(
        `${BROKER_BASE_URL}/api/v1/broker/events/${eventId}/tickets/disconnect`,
        null,
        {
            headers: {
                Authorization: `Bearer ${accessToken}`,
            },
            tags: { name: 'disconnect' },
    ...
    const ok = check(res, {
        'disconnect status 200': (r) => r.status === 200,
    });
...
export async function setup() {
    ...
    for (const eventId of EVENT_IDS) {
        ...
        let idx;
        try {
            waitingCount = await getWaitingKeyCount(waitingKey);
            userIdsCount = Number(await redisClient.hlen(userIdsKey)) || 0;
            idx = await safeHgetNumber('WAITING_QUEUE_IDX', eventId);
        } catch (e) {
            fail(`Redis precondition check failed: ${e}`);
        }
        if (waitingCount > 0 || userIdsCount > 0 || idx > 0) {
            fail(
                `Precondition failed: leftover queue state for eventId=${eventId} ` +
                `(${waitingKey}=${waitingCount}, HLEN ${userIdsKey}=${userIdsCount}, HGET WAITING_QUEUE_IDX[${eventId}]=${idx})`
            );
        }
    }
...
export async function adminController(data) { // Explicitly exported
    console.log('--- Admin Controller: Waiting until all VUs entered waiting endpoint (WAITING_QUEUE_IDX barrier) ---');
    const adminJar = http.cookieJar();
    const adminSession = loginUserWithRetry(ADMIN_EMAIL, ADMIN_PASSWORD, adminJar);
    ...
    while (true) {
        const total = await getWaitingQueueIdxTotal();
        if (Number(total) >= expected) {
            break;
        }
        if (Date.now() - start > BARRIER_TIMEOUT_MS) {
            const counts = await getWaitingCountsByEvent();
            fail(`Barrier timeout: waitingQueueIdxTotal=${total}, expected=${expected}, waitingByEvent=${JSON.stringify(counts)}`);
        }
...
export default function (data) {
    ...
    connectSse(eventId, session.accessToken, jar, data?.runId);
    // completedUsers.add(1) is now handled inside connectSse's event handler
    // sseFailures are handled inside connectSse as well
}


export function handleSummary(data) {
    const completed = data.metrics.completed_users?.values?.count || 0;
    const inEntryEvents = data.metrics.in_entry_events?.values?.count || 0;
    const sseEvents = data.metrics.sse_events?.values?.count || 0;
    ...
    const sseFailuresCount = data.metrics.sse_failures?.values?.count || 0;
