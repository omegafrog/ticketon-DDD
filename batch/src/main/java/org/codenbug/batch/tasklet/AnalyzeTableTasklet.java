...
public class AnalyzeTableTasklet implements Tasklet {
    ...
    public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) {
        ...
        log.info("Starting ANALYZE TABLE {} at {}", tableName,
        ...
        try {
            ...
            Integer tableExists =
                    jdbcTemplate.queryForObject(checkTableSql, Integer.class, tableName);
            ...
            log.info("ANALYZE TABLE {} completed successfully in {}ms at {}", tableName,
                    analyzeDuration, endTime.format(DateTimeFormatter.ISO_LOCAL_DATE_TIME));
            ...
            try {
                ...
                jdbcTemplate.queryForList(statsSql, tableName).forEach(row -> {
                    log.info(
                            "Updated statistics for {}: rows={}, avg_row_length={}, data_length={}, index_length={}, update_time={}",
                            tableName, row.get("table_rows"), row.get("avg_row_length"),
                            row.get("data_length"), row.get("index_length"),
                            row.get("update_time"));
                });

            } catch (Exception e) {
                log.warn("Failed to retrieve updated statistics for {}: {}", tableName,
                        e.getMessage());
            }
            ...
            if (e.getMessage() != null) {
                if (e.getMessage().contains("timeout")) {
                    log.error("ANALYZE TABLE {} timed out after {} seconds", tableName,
                            timeoutSeconds);
                } else if (e.getMessage().contains("Access denied")) {
                    log.error(
                            "Access denied for ANALYZE TABLE {}. Please check database permissions.",
                            tableName);
            ...
            throw new RuntimeException(
                    "ANALYZE TABLE " + tableName + " failed with unexpected error", e);
        }
