 ...
 * 비즈니스 로직에 사용되면서 유저에게 노출되는 데이터는 {@link EventInformation}, 그렇지 않은 데이터는
 * {@link MetaData}에 포함됨
...
public class Event {
  ...
  public void update(EventInformation information) {
    update(information, LocalDateTime.now());
  }

  public void update(EventInformation information, LocalDateTime now) {
    verifyBookingNotEnded(now);
    information.validate();
    if (isPaymentRelevantChanged(this.eventInformation, information)) {
      verifySellingEventPaymentRelevantFieldsUnchanged(information, now);
      incrementSalesVersion();
    }
    this.eventInformation = information;
  }

  private void verifyBookingNotEnded(LocalDateTime now) {
    if (now.isAfter(this.eventInformation.getBookingEnd()))
      throw new IllegalStateException("Cannot update event after booking is end");
  }

  private void verifySellingEventPaymentRelevantFieldsUnchanged(EventInformation information, LocalDateTime now) {
    if (this.eventInformation.isBookableAt(now)) {
      throw new IllegalStateException("Cannot update payment-relevant event fields while event is selling");
    }
  }
  ...
  public void delete() {
    if (this.eventInformation.getStatus() != EventStatus.DELETED) {
      incrementSalesVersion();
    }
    this.eventInformation = new EventInformation(
        this.eventInformation.getTitle(),
