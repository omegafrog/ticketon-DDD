...
public class MonitoringController {
    ...
    public ResponseEntity<Map<String, Object>> getThreadPoolStatus() {
        ...
        try {
            ...
            Set<ObjectName> threadPoolNames = server.queryNames(new ObjectName("Tomcat:type=ThreadPool,name=*"), null);
            ...
            // Executor MBeans도 확인
            Set<ObjectName> executorNames = server.queryNames(new ObjectName("Tomcat:type=Executor,name=*"), null);
                ...
                for (ObjectName executorName : executorNames) {
                    try {
                        ...
                        if (activeCount != null && maximumPoolSize != null) {
                            ...
                            String status = utilization < 50 ? "HEALTHY" : utilization < 80 ? "BUSY" : "OVERLOADED";
                            summary.put("status", status);
