...
public class EntryAuthService {
  ...
  public String generateEntryAuthToken(Map<String, Object> claims, String subject) {
    return Jwts.builder().signWith(Util.Key.convertSecretKey(secret)).setClaims(claims)
        .claim("subject", subject).setExpiration(Date.from(Instant.now().plusSeconds(expiration)))
        .compact();
  }
