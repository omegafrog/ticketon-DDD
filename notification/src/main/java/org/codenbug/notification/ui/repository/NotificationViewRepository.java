...
public interface NotificationViewRepository {
     ...
     * 사용자별 미읽은 알림 리스트 조회
     */
    Page<NotificationListProjection> findUserUnreadNotificationList(String userId, Pageable pageable);
    
    /**
     * 사용자별 특정 타입 알림 리스트 조회
     */
    Page<NotificationListProjection> findUserNotificationListByType(String userId, String type, Pageable pageable);
    
    /**
     * 커서 기반 페이징으로 알림 리스트 조회
     */
    List<NotificationListProjection> findUserNotificationListWithCursor(String userId, Long cursor, int size);
