...
public class NotificationViewRepositoryImpl implements NotificationViewRepository {
    ...
    public NotificationViewRepositoryImpl(
    ...
    public Page<NotificationListProjection> findUserNotificationList(String userId,
        ...
        List<NotificationListProjection> results = queryFactory
                .select(Projections.constructor(NotificationListProjection.class, notification.id,
                        ...
                        notification.isRead, notification.status.stringValue(),
                        notification.sentAt))
                .from(notification).where(notification.userId.value.eq(userId))
                .offset(pageable.getOffset()).limit(pageable.getPageSize())
                .orderBy(notification.sentAt.desc()).fetch();

        Long total = queryFactory.select(notification.count()).from(notification)
                .where(notification.userId.value.eq(userId)).fetchOne();
    ...
    public Page<NotificationListProjection> findUserNotificationListByType(String userId,
        ...
        List<NotificationListProjection> results = queryFactory
                ...
                .where(notification.userId.value.eq(userId)
                        .and(StringUtils.hasText(type)
                                ? notification.type.eq(NotificationType.valueOf(type))
                                : null))
                .offset(pageable.getOffset()).limit(pageable.getPageSize())
                .orderBy(notification.sentAt.desc()).fetch();
    ...
    public List<NotificationListProjection> findUserNotificationListWithCursor(String userId,
        ...
        return queryFactory
                ...
                .orderBy(notification.id.asc()).limit(size).fetch();
    }

    @Override
    public long countUnreadNotifications(String userId) {
        Long count = queryFactory.select(notification.count()).from(notification)
                .where(notification.userId.value.eq(userId).and(notification.isRead.isFalse()))
                .fetchOne();
