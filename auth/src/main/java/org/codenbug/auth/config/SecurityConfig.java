...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity http, ObjectMapper objectMapper)
		...
		http.authorizeHttpRequests(authorize -> authorize.requestMatchers("/**").permitAll())
				.formLogin(config -> config.disable()).csrf(config -> config.disable()).exceptionHandling(
						config -> config.authenticationEntryPoint((request, response, authException) -> {
							...
							response.getWriter().write(objectMapper
									.writeValueAsString(new RsData<>("403", authException.getMessage(), null)));
							...
							response1.getWriter().write(objectMapper.writeValueAsString(
